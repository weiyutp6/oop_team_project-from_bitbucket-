...
public class TripPanel extends JPanel {
	...
	public TripPanel(Object[] tripOfList) {
		...
		relateButton = new JButton("查看相關行程");
		btnPanel = new JPanel();
		
		btnPanel.setLayout(new FlowLayout());
		
		btnPanel.add(relateButton);
		...
		relateButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(clicked == relateButton) {
					if(textListener != null) {
						textListener.textEmitted("relateBtnInTrip\n");
					}
				}
			}
		});
				...
				if(table.getSelectedRow()==-1) {
					...
					if(clicked == orderButton) {
						if(textListener != null) {
							...
							textListener.textEmitted("orderBtnInTrip\n");
						}
					}
			    }
			}
